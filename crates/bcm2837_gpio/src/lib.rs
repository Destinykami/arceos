...
impl RpiGpioPort {
    ...
    pub fn direction_output(&mut self, offset: u32)  {
    ...
    /// get gpio port value
    pub fn get_value(&mut self, offset: u32) -> u32 {
        let lev_register = match offset as usize / 32 {
            0 => &self.regs().GPLEV0,
            1 => &self.regs().GPLEV1,
            _=> unreachable!(),
        };
        // Read the pin value
        let value = (lev_register.get() >> (offset % 32)) & 0x1;
        value
    }
    ///init
    pub fn gpio_init(&mut self) {
        self.direction_output(17);
    }
}
